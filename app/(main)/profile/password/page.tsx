...
export default function ChangePasswordPage() {
  ...
  const [validationErrors, setValidationErrors] = useState<{ currentPassword?: string; newPassword?: string; confirmPassword?: string }>({});
            ...
            <div className="glass-card rounded-[3.5rem] border border-black/5 bg-white p-10 lg:p-16 animate-in slide-in-from-bottom-12 duration-700 shadow-2xl shadow-black/5 max-w-3xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-10">
                    <div className="group space-y-3">
                      <label className={`text-[10px] font-black uppercase tracking-[0.4em] ml-4 transition-colors ${validationErrors.currentPassword ? "text-red-500" : "text-[#0B1221]/40 group-focus-within:text-blue-600"}`}>
                        Current Password *
                      </label>
                      <div className="relative">
                        <LockIcon size={18} className={`absolute left-6 top-1/2 -translate-y-1/2 transition-all duration-500 ${validationErrors.currentPassword ? "text-red-500" : "text-[#0B1221]/10 group-focus-within:text-blue-600"}`} strokeWidth={1.5} />
                        <input
                          type="password"
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        <div className="group space-y-3">
                            <label className={`text-[10px] font-black uppercase tracking-[0.4em] ml-4 transition-colors ${validationErrors.newPassword ? "text-red-500" : "text-[#0B1221]/40 group-focus-within:text-blue-600"}`}>
                                New Password *
                            </label>
                            <div className="relative">
                                <span className={`absolute left-6 top-1/2 -translate-y-1/2 transition-all duration-500 ${validationErrors.newPassword ? "text-red-500" : "text-[#0B1221]/10 group-focus-within:text-blue-600"}`}>
                        ...
                        <div className="group space-y-3">
                            <label className={`text-[10px] font-black uppercase tracking-[0.4em] ml-4 transition-colors ${validationErrors.confirmPassword ? "text-red-500" : "text-[#0B1221]/40 group-focus-within:text-blue-600"}`}>
                                Confirm Password *
                            </label>
                            <div className="relative">
                                <span className={`absolute left-6 top-1/2 -translate-y-1/2 transition-all duration-500 ${validationErrors.confirmPassword ? "text-red-500" : "text-[#0B1221]/10 group-focus-within:text-blue-600"}`}>
                                    <LockIcon size={18} strokeWidth={1.5} />
                                </span>
                                <input
                                    type="password"
