      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-24 -mt-12 sm:-mt-20 relative z-20">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-8 sm:mb-12 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
          {CONTACT_INFO.map(({ icon: Icon, label, value, href }) => (
            <div key={label} className="bg-white rounded-[2rem] p-6 sm:p-8 shadow-xl shadow-black/[0.03] border border-black/5 text-center group hover:-translate-y-2 hover:shadow-2xl transition-all duration-500">
              ...
              {href ? (
                <a href={href} className="text-sm font-bold text-[#0B1221] hover:text-cyan-600 transition-colors break-words">
                  {value}
                </a>
              ) : (
                <p className="text-sm font-bold text-[#0B1221] break-words">{value}</p>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-[2rem] sm:rounded-[3rem] border border-black/5 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] p-8 sm:p-14 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500">
          {submitted ? (
            <div className="text-center py-16 animate-in zoom-in duration-500">
              ...
              <h2 className="hero-display text-4xl text-[#0B1221] mb-4">Message Sent!</h2>
              <p className="text-gray-500 text-lg max-w-sm mx-auto">We&apos;ll get back to you within 24 hours. Thank you for reaching out.</p>
            </div>
          ) : (
            <>
              <div className="text-center mb-10">
                <h2 className="hero-display text-3xl sm:text-4xl text-[#0B1221]">Send a Message</h2>
              </div>
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-6 max-w-3xl mx-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input required value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
                      placeholder="John Doe" className="w-full px-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-4">Email Address *</label>
                    <input required type="email" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))}
                      placeholder="contact@example.com" className="w-full px-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all" />
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest pl-4">Phone</label>
                    <input type="tel" value={form.phone} onChange={(e) => setForm((f) => ({ ...f, phone: e.target.value }))}
                      placeholder="Optional" className="w-full px-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all" />
                  </div>
                  <div className="space-y-2">
                    ...
                    <input required value={form.subject} onChange={(e) => setForm((f) => ({ ...f, subject: e.target.value }))}
                      placeholder="How can we help?" className="w-full px-6 py-4 bg-gray-50/50 border border-gray-100 rounded-2xl text-sm font-medium outline-none focus:bg-white focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 transition-all" />
                  </div>
                </div>
                <div className="space-y-2">
                ...
                <div className="pt-4">
                  <button type="submit" className="w-full md:w-auto md:px-14 bg-[#0B1221] text-white py-4 sm:py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-cyan-600 transition-colors hover:shadow-xl hover:shadow-cyan-600/20 mx-auto block">
                    Send Message
                  </button>
                </div>
              </form>
