      ...
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
          {STORES.map((store, idx) => {
            const initial = store.name.split("-")[1]?.trim().slice(0, 3).toUpperCase() || "BS";
            return (
              <div 
                key={store.name} 
              ...
              >
                ...
                <div className="relative z-10">
                    ...
                    <div className="space-y-4 mb-10">
                      <h2 className="hero-display text-2xl tracking-tighter text-[#0B1221] uppercase">{store.name}</h2>
                      <p className="text-xs font-bold text-[#0B1221]/40 leading-relaxed uppercase tracking-tight">{store.address}</p>
                    </div>
                    ...
                    <div className="space-y-4 py-8 border-y border-black/5 mb-10">
                      <div className="flex items-center gap-3 group/link">
                        ...
                        <a href={`tel:${store.phone.replace(/-/g, "")}`} className="text-[10px] font-black text-[#0B1221]/40 uppercase tracking-widest group-hover/link:text-blue-600 transition-colors">
                          {store.phone}
                        </a>
                      </div>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="space-y-1">
                             <span className="text-[10px] font-black text-[#0B1221]/20 uppercase tracking-widest block">Available Window</span>
                             <span className="text-[10px] font-black text-[#0B1221]/60 uppercase tracking-widest">{store.hours}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <Navigation size={16} strokeWidth={2} className="group-hover:rotate-12 transition-transform duration-500" />
                      Establish Route
                    </a>
                </div>
              </div>
