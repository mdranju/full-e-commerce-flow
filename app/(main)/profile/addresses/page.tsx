            ...
            {showForm && (
              <div className="glass-card rounded-[3.5rem] border border-blue-500/10 bg-white p-10 lg:p-16 animate-in slide-in-from-bottom-12 duration-700 shadow-2xl shadow-blue-500/5">
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-12">
                  <div>
                      <p className="text-blue-600 text-[10px] font-black tracking-[0.4em] uppercase mb-4">Add Address</p>
                      <h2 className="hero-display text-4xl lg:text-5xl tracking-tighter text-[#0B1221]">
                        New Address.
                      </h2>
                  </div>
                  <button onClick={() => setShowForm(false)} className="p-3 hover:bg-gray-100 rounded-2xl transition-colors">
            ...
            {addresses.length === 0 ? (
              <div className="text-center py-40 glass-card rounded-[3.5rem] bg-white border border-black/5 flex flex-col items-center">
                <div className="w-24 h-24 rounded-full bg-gray-50 flex items-center justify-center mb-8">
                    <MapPin size={32} className="text-[#0B1221]/10" strokeWidth={1} />
                </div>
                <h3 className="hero-display text-3xl tracking-tighter text-[#0B1221] mb-2 uppercase">No Addresses Found</h3>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {addresses.map((address, idx) => (
                  <div key={address.id} className="animate-in fade-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: `${idx * 100}ms` }}>
                    {editingId === address.id ? (
                       <div className="glass-card rounded-[3rem] border border-blue-500/20 bg-white p-10 shadow-2xl shadow-blue-500/5">
                        <div className="flex items-center justify-between mb-8">
                            <h2 className="text-sm font-black text-[#0B1221] tracking-tight uppercase">Edit Address</h2>
                            <button onClick={() => setEditingId(null)} className="p-2 hover:bg-gray-50 rounded-xl">
                      ...
                      <div className={`group relative glass-card p-10 rounded-[3rem] border transition-all duration-700 hover:shadow-2xl hover:shadow-black/5 hover:-translate-y-1 ${address.isDefault ? "border-blue-600/20 bg-white ring-1 ring-blue-600/5" : "border-black/5 bg-white"}`}>
                        <div className="flex items-start justify-between gap-6">
                          <div className="flex items-start gap-6 flex-1 min-w-0">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform duration-700 group-hover:scale-110 group-hover:shadow-2xl ${address.isDefault ? "bg-blue-600 text-white shadow-blue-500/20 shadow-xl" : "bg-gray-50 text-[#0B1221]/20 shadow-sm"}`}>
                              {address.label === "Home" ? <Home size={22} strokeWidth={1.5} /> : address.label === "Office" ? <Briefcase size={22} strokeWidth={1.5} /> : <MapPin size={22} strokeWidth={1.5} />}
                            </div>
                            <div className="min-w-0">
                              <div className="flex items-center gap-3 mb-2 flex-wrap">
                                <span className={`text-[10px] font-black uppercase tracking-[0.4em] ${address.isDefault ? "text-blue-600" : "text-[#0B1221]/40"}`}>
                                ...
                                {address.isDefault && (
                                  <div className="flex items-center gap-1.5 px-3 py-1 bg-blue-600/5 border border-blue-600/10 rounded-full animate-pulse">
                                      ...
                                      <span className="text-[8px] font-black text-blue-600 uppercase tracking-widest">Default</span>
                                  </div>
                                )}
                              </div>
                              ...
                              <div className="space-y-1">
                                  <p className="text-xs font-bold text-[#0B1221]/60 leading-relaxed uppercase tracking-tight">
                                    ...
                                    {address.city}, {address.district} {address.postalCode && `- ${address.postalCode}`}
                                  </p>
                                  <p className="text-[10px] font-black tracking-[0.2em] text-[#0B1221]/20 uppercase pt-2">Phone: {address.phone}</p>
                              </div>
                            </div>
                          </div>
                        </div>
                             ...
                             <div className="flex gap-4">
                                ...
                                {address.isDefault && (
                                    <div className="flex items-center gap-2 text-blue-600 text-[10px] font-black uppercase tracking-widest">
                                        <Star size={14} fill="currentColor" /> Default Address
                                    </div>
                                )}
                             </div>
